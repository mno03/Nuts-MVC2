function go_cart() {
	if (document.formm.quantity.value == "") {
		...
		document.formm.action = "NutsServlet?command=cart_insert"
...
function go_cart_delete() {
	...
	if (count == 0) {
		...
		document.formm.action = "NutsServlet?command=cart_delete"
		document.formm.submit();
	}
}
function go_order_insert() {
	document.formm.action = "NutsServlet?command=order_insert"
	document.formm.submit();
}
function go_order_delete() {
	...
	if (count == 0) {
		...
		document.formm.action = "NutsServlet?command=order_delete"
		document.formm.submit();
	}
} function go_order() {
	document.formm.action = "NutsServlet?command=mypage"
	document.formm.submit();
}
...
function go_mov() {
	var theForm = document.frm;
	theForm.action = "NutsServlet?command=admin_product_list";
	theForm.submit();
}
//projectList.jsp
function go_search() {
...
function go_detail(tpage, pseq) {
	...
	theForm.action = "NutsServlet?command=admin_product_detail&tpage=" +
		tpage + "&pseq=" + pseq;
...
function go_wrt() {
	var theForm = document.frm;
	theForm.action = "NutsServlet?command=admin_product_write_form";
	theForm.submit();
}
// **************** productDetail.jsp
function go_list(tpage) {
	...
	theForm.action = "NutsServlet?command=admin_product_list&tpage=" + tpage;
	theForm.submit();
}
// **************** productDetail.jsp
function go_mod(tpage, pseq) {
	...
	theForm.action = "NutsServlet?command=admin_product_update_form&tpage=" +
		tpage + "&pseq=" + pseq;
	theForm.submit();
}
...
function go_mod_save(tpage, pseq) {
	...
	if (theForm.kind.value == '') {
		...
		if (confirm('수정하시겠습니까?')) {
			...
			theForm.action = "NutsServlet?command=admin_product_update";
			theForm.submit();
		}
	}
}
...
function go_mod_mov(tpage, pseq) {
	var theForm = document.frm;
	theForm.action = 'NutsServlet?command=admin_product_detail&tpage=' + tpage + "&pseq=" + pseq;
	theForm.submit();
}
