...
function go_save() {
	if (document.formm.id.value == "") {
		...
		document.formm.action = "NutsServlet?command=join";
...
function idcheck() {
	...
	var url = "NutsServlet?command=id_check_form&id="
		+ document.formm.id.value;
	window.open(url, "_blank_1",
		"toolbar=no, menubar=no, scrollbars=yes, resizable=no, width=330, height=200");
}
function post_zip() {
	var url = "NutsServlet?command=find_zip_num";
	window.open(url, "_blank_1",
		"toolbar=no, menubar=no, scrollbars=yes, resizable=no, width=330, height=200");
}
function go_next() {
	if (document.formm.okon1[0].checked == true) {
		document.formm.action = "NutsServlet?command=join_form";
		document.formm.submit();
